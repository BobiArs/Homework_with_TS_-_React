# Крок 5: Збірка сторінки та роутинг

На цьому фінальному кроці ми створимо головну сторінку `HomePage`, де об'єднаємо фільтри, анімацію рулетки та модальне вікно. Також налаштуємо маршрутизацію React Router, QueryClient та протестуємо повну збірку додатку.

---

## 1. Створення головної сторінки (`src/pages/home/ui/HomePage.tsx`)

Створіть директорію `src/pages/home/ui/` та додайте файл `HomePage.tsx`. 

Тут знаходиться логіка мапінгу фільтрів у параметри TMDB, запуск спіннера та вибір випадкового фільму:
...
export const HomePage: React.FC = () => {
  ...
  // Допоміжна функція мапінгу фільтрів на API параметри TMDB
  const buildApiParams = (f: FilterState) => {
    const params: Parameters<typeof discoverMovies>[0] = {};

    // 1. Мапінг настрою на ID жанрів
    if (f.mood === 'joyful') params.with_genres = '35,16'; // Комедія, Анімація
    else if (f.mood === 'tense') params.with_genres = '53,9648,27'; // Трилер, Детектив, Жахи
    else if (f.mood === 'romantic') params.with_genres = '10749,18'; // Мелодрама, Драма
    else if (f.mood === 'epic') params.with_genres = '28,12,878'; // Бойовик, Пригоди, Фантастика
  ...
  const handleStartSpin = async () => {
    ...
    try {
      ...
      // Робимо динамічний запит до TMDB API
      ...
      // Беремо топ-12 популярних фільмів для сегментів колеса
      const candidates = movies.slice(0, 12);
      setMoviesList(candidates);
      setIsLoading(false);
      
      // Запускаємо анімацію колеса
      setIsSpinning(true);
    } catch (e) {
      console.error(e);
...
## 2. Створення публічного API для сторінки (`src/pages/home/index.ts`)

Створіть файл `index.ts` в папці `src/pages/home/`:
...
## 3. Налаштування роутингу (`src/app/route.tsx`)

Замініть вміст `src/app/route.tsx` для підключення нашої сторінки:
...
## 4. Головна точка входу (`src/app/main.tsx`)

Переконайтеся, що файл `src/app/main.tsx` правильно огортає додаток у `QueryClientProvider`:
...
## 5. Запуск та фінальна збірка

Запустіть процес збірки для перевірки відсутності помилок TypeScript:

```bash
npm run build
```

Для локальної перевірки запустіть dev-сервер:

```bash
npm run dev
```

Перейдіть за адресою `http://localhost:5173/` та протестуйте роботу! Для підключення ваших даних обов'язково налаштуйте змінну оточення `.env` з вашим TMDB Bearer токеном:

```env
VITE_TMDB_ACCESS_TOKEN=ваш_токен_тут
```

Вітаємо, ви створили захоплюючу та стильну кіно-рулетку! 🎡🎬
